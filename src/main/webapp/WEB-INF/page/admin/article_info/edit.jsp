									...
									<div class="right">
										<select id="type_id" name="typeId">
											<c:forEach items="${typeList}" var="typeInfo" varStatus="status" >
												<option value="${typeInfo.id}" <c:if test="${articleInfo.typeId==typeInfo.id}">selected="selected"</c:if>>${typeInfo.name}</option>
												<%-- <c:choose>
													<c:when test="${articleInfo.typeId==typeInfo.id}">
														<!-- 说明此处是编辑文章，有id -->
														<option value="${typeInfo.id}" selected="selected">${typeInfo.name}</option>
													</c:when>
													<c:otherwise>
...
<script>
	...
	javaex.upload({
		...
		callback : function (rtn) {
			...
			// 后台返回的数据
			if (rtn.code=="000000") {
				var imgUrl = "/upload/"+rtn.data.imgUrl;
				/* console.log(imgUrl);
	...
	javaex.edit({
		...
		callback : function(rtn) {
			...
			$("#contentText").val(rtn.text.substring(0, 100));//前台截取前100个字符
		}
	});
